<script>
	...
	let {
		...
		/** Label shown in the debugger panel to identify this scroller instance. */
		ref = '',
		index = $bindable(0),
		count = $bindable(0),
		offset = $bindable(0),
	...
	let debuggerTop = $state(16);
	let debuggerRight = $state(16);
	...
	function update() {
		...
		if (showDebugger) {
			debuggerVisible = bcr.bottom > 0 && bcr.top < wh;
			debuggerTop = 16;
			debuggerRight = Math.max(16, ww - bcr.right + 16);
		} else {
...
<svelte:window bind:innerHeight={wh} bind:innerWidth={ww} onscroll={update} onresize={update} />

<div class="scroller-root">
...
{#if showDebugger && debuggerVisible}
	<div
		class="debugger"
		style:top="{debuggerTop}px"
		style:right="{debuggerRight}px"
		aria-hidden="true"
	>
		<p class="debugger-title">{ref || 'scroller'}</p>
